...
public class ftRenderLightmap : EditorWindow//ScriptableWizard
{
        ...
        if (gstorage != null)
        {
            ...
            else
            {
                if (!rtxMode && !gstorage.runsNonRTX)
                {
                    ...
                    {
                        Debug.Log("Automatically enabled RTX");
                        rtxMode = true;
                ...
                {
                    DebugLogError("This scene has denoiser set to OptiX 5, but you GPU does not seem to support it. Please change the denoiser or re-run Bakery -> Utilities -> Detect optimal settings.");
                    return false;
                }
                else if (denoiserType == ftGlobalStorage.DenoiserType.Optix6 && !gstorage.runsOptix6)
                {
                    DebugLogError("This scene has denoiser set to OptiX 6, but you GPU does not seem to support it. Please change the denoiser or re-run Bakery -> Utilities -> Detect optimal settings.");
                    return false;
                }
                else if (denoiserType == ftGlobalStorage.DenoiserType.Optix7 && !gstorage.runsOptix7)
                {
                    DebugLogError("This scene has denoiser set to OptiX 7, but you GPU does not seem to support it. Please change the denoiser or re-run Bakery -> Utilities -> Detect optimal settings.");
                    return false;
                }
                else if (denoiserType == ftGlobalStorage.DenoiserType.OpenImageDenoise && !gstorage.runsOIDN)
                {
                    DebugLogError("This scene has denoiser set to OpenImageDenoise, but you CPU does not seem to support it. Please change the denoiser or re-run Bakery -> Utilities -> Detect optimal settings.");
                    return false;
                }
            }
        }
    ...
    public void RenderButton(bool showMsgWindows = true)
    {
        ...
        else
        {
            if (!foundCompatibleSetup && verbose)
            {
                ...
                if (EditorUtility.DisplayDialog("Bakery", "Would you like to automatically detect optimal settings for your hardware? You can also do it later via Bakery/Utilities/Detect optimal settings", "Yes", "No"))
                {
    ...
    void OnGUI()
    {
        ...
        GUI.Label(aboutRect, new GUIContent("<color=#5073c9ff><b>Bakery - GPU Lightmapper by Mr F</b></color>", "Version 1.8. Go to Bakery Wiki"), linkStyle);
    ...
    public static BakeryVolume[] FindBakeableVolumes()
    {
        ...
        {
            if (vols[v].enableBaking) vols2.Add(vols[v]);
        }
        ...
        try
        {
            ...
            if (ok)
            {
                ...
                for(int v=0; v<vols.Length; v++)
                {
                    ...
                    var texData0 = new Color[numProbes];
                    var texData1 = new Color[numProbes];
                    var texData2 = new Color[numProbes];
                    for(int z=0; z<rz; z++)
                    {
                        for(int y=0; y<ry; y++)
                        {
                    ...
                    var vformat = vol.encoding == BakeryVolume.Encoding.Half4 ? TextureFormat.RGBAHalf : TextureFormat.ARGB32;

                    var outName = outNameBase + "0.asset";
                    if (File.Exists(outName)) ValidateFileAttribs(outName);
                    var tex = new Texture3D(rx, ry, rz, vformat, false);
                    ...
                    if (shadowmask)
                    {
                        ...
                        vformat = (vol.shadowmaskEncoding == BakeryVolume.ShadowmaskEncoding.A8) ? TextureFormat.Alpha8 : TextureFormat.ARGB32;
    ...
    IEnumerator RenderLightProbesFunc()
    {
        ...
        Debug.Log("Multi-threading to " + maxThreads + " threads.");
        ...
        bakeInProgress = false;
        Debug.Log("Finished rendering Light Probes.");
        yield break;
    }
                ...
                if (lmgroup.probes && lmgroup.name == "volumes" && lastFoundBakeableVolumes != null && lastFoundBakeableVolumes.Length > 0)
                {
                    ...
                    for(int v=0; v<vols.Length; v++)
                    {
                        ...
                        if (vol.denoise)
                        {
                            ...
                            startInfo.Arguments      += "\"" + scenePath + "/volumes_final_L0" + (compressedOutput ? ".lz4" : ".dds") +
                                                     "\" \"" + scenePath + "/volumes_final_L1x" + (compressedOutput ? ".lz4" : ".dds") +
                                                     "\" \"" + scenePath + "/volumes_final_L1y" + (compressedOutput ? ".lz4" : ".dds") +
                                                     "\" \"" + scenePath + "/volumes_final_L1z" + (compressedOutput ? ".lz4" : ".dds") +
                                                     "\" " +
    ...
    IEnumerator ApplyBakedData()
    {
        ...
        foreach(var lmgroup in groupListGIContributingPlain)
        {
            ...
            {
                outfile = "Assets/" + outputPath + "/"+nm+"_mask" + (ftAdditionalConfig.preferPNG ? ".png" : ".tga");
            ...
            if (dominantDirMode)
            {
                outfile = "Assets/" + outputPath + "/"+nm+"_dir" + (ftAdditionalConfig.preferPNG ? ".png" : ".tga");
                if (File.Exists(outfile))
                {
            ...
            if (shMode)
            {
                ...
                for(int c=0; c<3; c++)
                {
                    ...
                    outfile = "Assets/" + outputPath + "/"+nm+"_L1" + comp + (ftAdditionalConfig.preferPNG ? ".png" : ".tga");
                    if (c == 0) rnmLightmap0 = AssetDatabase.LoadAssetAtPath(outfile, typeof(Texture2D)) as Texture2D;
                    if (c == 1) rnmLightmap1 = AssetDatabase.LoadAssetAtPath(outfile, typeof(Texture2D)) as Texture2D;
                    if (c == 2) rnmLightmap2 = AssetDatabase.LoadAssetAtPath(outfile, typeof(Texture2D)) as Texture2D;
                }
        ...
        foreach(var lmgroup in groupListGIContributingPlain)
        {
            ...
            //if (!bc6h)
            {
                //if (File.Exists(folder + "../Assets/" + nm + "_final.hdr"))
                {
                    ...
                    if (lightmapHasMask[lmgroup.id])
                    {
                        outfile = "Assets/" + outputPathCompat + "/"+nm+"_mask" + (ftAdditionalConfig.preferPNG ? ".png" : ".tga");
                    ...
                    if (dominantDirMode)
                    {
                        outfile = "Assets/" + outputPathCompat + "/"+nm+"_dir" + (ftAdditionalConfig.preferPNG ? ".png" : ".tga");
                    ...
                    if (shMode)
                    {
                        ...
                        for(int c=0; c<3; c++)
                        {
                            ...
                            outfile = "Assets/" + outputPathCompat + "/"+nm+"_L1" + comp + (ftAdditionalConfig.preferPNG ? ".png" : ".tga");
                            ftTextureProcessor.texSettings[outfile] = new Vector2(dirSize, desiredTextureType);
                        }
                    }
                }
    ...
    IEnumerator RenderLMFinalize(int LMID, string lmname, int resolution, bool vertexBake, int lmgroupRenderDirMode, int lmgroupRenderMode, BakeryLightmapGroupPlain lmgroup)
    {
        ...
        if (userRenderMode == RenderMode.Shadowmask)
        {
            ...
            if (maskNames.Count > 0 && process)
            {
                ...
                var outPath = Application.dataPath + "/" + outputPath + "/" + lmname + "_mask" + (ftAdditionalConfig.preferPNG ? ".png" : ".tga");
                if (File.Exists(outPath)) ValidateFileAttribs(outPath);
                ...
                if (deferredMode)
                {
                    ...
                    deferredCommands.Add(startInfo);
                    deferredCommandDesc.Add(progressText);
                    if (clientMode && !vertexBake) ftClient.serverGetFileList.Add(lmname + "_mask" + (ftAdditionalConfig.preferPNG ? ".png" : ".tga"));
                }
                else
                {
    ...
    IEnumerator RenderLMEncode(int LMID, string lmname, int resolution, bool vertexBake, int lmgroupRenderDirMode, int lmgroupRenderMode)
    {
        ...
        {
            var outPath = Application.dataPath + "/" + outputPath + "/" + lmname + "_dir" + (ftAdditionalConfig.preferPNG ? ".png" : ".tga");
            if (File.Exists(outPath)) ValidateFileAttribs(outPath);
            ...
            startInfo.CreateNoWindow = true;
            startInfo.Arguments       = "\"" + scenePath + "/" + lmname + "_final_Dir" + (compressedOutput ? ".lz4" : ".dds") + "\" \"" + outPath + "\" " + (ftAdditionalConfig.preferPNG ? "p" : "");
            ...
            if (deferredMode)
            {
                ...
                deferredCommands.Add(startInfo);
                deferredCommandDesc.Add(progressText2);
                if (clientMode) ftClient.serverGetFileList.Add(lmname + "_dir" + (ftAdditionalConfig.preferPNG ? ".png" : ".tga"));
            }
            else
            {
                Debug.LogError("Not supported");
        ...
        if (shMode && !vertexBake && lightmapHasRNM[LMID])
        {
            ...
            for(int i=0; i<3; i++)
            {
                ...
                var outPath1 = Application.dataPath + "/" + outputPath + "/" + lmname + "_L1" + comp + (ftAdditionalConfig.preferPNG ? ".png" : ".tga");
                if (File.Exists(outPath1)) ValidateFileAttribs(outPath1);

                startInfo.Arguments       = "\"" + scenePath + "/" + lmname + "_final_L1" + comp + (compressedOutput ? ".lz4" : ".dds") + "\" \"" + outPath1 + "\" " + (ftAdditionalConfig.preferPNG ? "p" : "");
                ...
                if (deferredMode)
                {
                    ...
                    deferredCommands.Add(startInfo);
                    deferredCommandDesc.Add(progressText2);
                    if (clientMode) ftClient.serverGetFileList.Add(lmname + "_L1" + comp + (ftAdditionalConfig.preferPNG ? ".png" : ".tga"));
                }
                else
                {
                    Debug.LogError("Not supported");
